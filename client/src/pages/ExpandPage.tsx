...
import { ArrowUp, GitBranch, PanelRightClose, PanelRightOpen, Play, Settings, Star } from 'lucide-react';
import { useParams, useSearchParams } from 'react-router-dom';
import { useStore } from '../stores/index.ts';
import { api } from '../lib/api.ts';
import type { TreeNode } from '../lib/tree.ts';
import type { PipelineType } from '../stores/agentSlice.ts';
import type { Run, RunEvent, RunWithEvents } from '../types/run.ts';
...
const VALIDATION_LEVELS: Array<{
...
}> = [
  {
    id: 'manual',
    label: 'Manual',
    description: 'Ask before proposal choices and before accepting generated output.',
  },
  {
    id: 'assisted',
    label: 'Assisted',
    description: 'Auto-select likely proposals, but keep user review before accepting.',
  },
  {
    id: 'autopilot',
    label: 'Autopilot',
...
const PHASE_LABELS: Record<string, string> = {
  idle: 'No active run',
  configuring: 'Ready to start',
  estimating: 'Estimating',
  generating: 'Generating',
  expanding: 'Expanding',
  proposals_ready: 'Needs selection',
  ideas_ready: 'Needs theme selection',
  reviewing: 'Needs review',
  continuing: 'Needs next-step choice',
  forging: 'Running',
  forge_done: 'Finished',
  done: 'Accepted',
  error: 'Error',
...
function agentPhaseNeedsInput(phase: string): boolean {
  return phase === 'proposals_ready' || phase === 'ideas_ready' || phase === 'reviewing' || phase === 'continuing';
}

function eventStatusClass(ok: boolean): string {
...
function eventStepStatus(event: RunEvent): { label: string; color: string } {
  return event.ok
    ? { label: 'Completed', color: 'bg-green-100 text-green-700' }
    : { label: 'Failed', color: 'bg-red-100 text-red-700' };
}

function eventOutputSummary(event: RunEvent): string {
  if (event.message) return event.message;
  if (event.ok) return `${event.step} completed for "${event.title}".`;
  return `${event.step} did not complete.`;
}
...
export default function ExpandPage() {
  ...
  useEffect(() => {
    if (!wid || !startingNodeId || agentPanelOpen) return;
    selectArticle(wid, startingNodeId).catch(console.error);
  }, [wid, startingNodeId, agentPanelOpen, selectArticle]);

  const loadRuns = async (selectLatest = false) => {
    if (!wid) return;
    const list = await api.runs.list(wid);
    setRuns(list);
    if (selectLatest && list[0]) setSelectedRunId(list[0].id);
    if (!selectedRunId && list[0]) {
      const active = list.find((run) => run.status === 'running' || run.status === 'pending' || run.status === 'paused');
  ...
  const activeRun = runs.find((run) => run.status === 'running' || run.status === 'pending' || run.status === 'paused') ?? null;
  const showPendingUserInputRun = agentPanelOpen && !forgeRunning && agentPhaseNeedsInput(agentPhase);
  ...
  const canClear = agentPanelOpen || forgeRunning || forgePaused || agentPhase === 'done' || agentPhase === 'error' || agentPhase === 'forge_done';
  ...
  const selectedRunLastEvent = selectedRunForDetails?.events[0] ?? null;
        ...
        <>
          <div className="px-4 py-3 border-b border-gray-100 flex items-start justify-between gap-3">
            ...
            <button
              ...
              className="text-[11px] text-gray-400 hover:text-gray-700 mt-0.5"
            >
              Clear
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3">
            {runs.length === 0 && !showPendingUserInputRun ? (
              <p className="text-xs text-gray-400">No generation runs yet.</p>
            ) : (
              <div className="space-y-2">
                {showPendingUserInputRun && (
                  <div className="rounded-lg border border-purple-100 bg-purple-50 p-3">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-[10px] px-1.5 py-0.5 rounded font-semibold uppercase bg-purple-100 text-purple-700">
                        Pending user input
                      </span>
                      <span className="text-[10px] text-purple-400">Current</span>
                    </div>
                    <div className="mt-2">
                      <p className="text-xs font-semibold text-purple-900">Manual Run</p>
                      <p className="text-xs text-purple-600 mt-0.5 truncate">
                        {PHASE_LABELS[agentPhase] ?? agentPhase}: {agentTargetArticleTitle ?? selectedNode?.title}
                      </p>
                    </div>
                    <p className="text-xs text-purple-600 mt-2">Step: {agentPipelineType}</p>
                  </div>
                )}
                {runs.map((run) => (
                  <button
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-[10px] text-gray-400">{formatTime(run.updatedAt)}</span>
                    </div>
                    <div className="mt-2">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  {activeRun && selectedRunId === activeRun.id
                    ? `${RUN_STATUS_LABELS[activeRun.status]} · ${activeRun.itemsCompleted} / ${activeRun.itemsTotal} items · updated ${formatTime(activeRun.updatedAt)}`
                    : selectedRunForDetails
                      ? `${RUN_STATUS_LABELS[selectedRunForDetails.status]} · updated ${formatTime(selectedRunForDetails.updatedAt)}`
            ...
            <section className="border border-gray-200 bg-white rounded-xl overflow-hidden min-h-[520px]">
              {selectedRunLoading ? (
                ...
                <div className="grid grid-cols-4 gap-3 p-4 border-b border-gray-100">
                  ...
                  <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                    <p className="text-[10px] uppercase tracking-wide text-gray-400">Budget Used</p>
                    <p className="text-sm font-semibold text-gray-900 mt-1">{selectedRunForDetails.budgetUsed.toLocaleString()} tokens</p>
                  </div>
                  <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                    <p className="text-[10px] uppercase tracking-wide text-gray-400">Last Step</p>
                    <p className="text-sm font-semibold text-gray-900 mt-1">{selectedRunLastEvent?.step ?? 'None'}</p>
                  </div>
                </div>
                ...
                <div className="p-4">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Step History</p>
                  {selectedRunForDetails.events.length === 0 ? (
                    <p className="text-xs text-gray-400">No events recorded yet.</p>
                  ) : (
                    <div className="space-y-2">
                      {selectedRunForDetails.events.map((event: RunEvent) => (
                        <div
                          key={event.id}
                          className={`rounded-lg border p-3 ${event.ok ? 'border-gray-200' : 'border-red-200 bg-red-50'}`}
                        >
                          <div className="flex items-center justify-between gap-3">
                            <div className="flex items-center gap-2 min-w-0">
                              <span className={`w-2 h-2 rounded-full shrink-0 ${eventStatusClass(event.ok)}`} />
                              <p className="text-xs font-semibold text-gray-800 truncate">{event.step}</p>
                              <p className="text-xs text-gray-500 truncate">{event.title}</p>
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                              <LabelBadge label={eventStepStatus(event).label} colorClass={eventStepStatus(event).color} />
                              <span className="text-[10px] text-gray-400">{formatTime(event.createdAt)}</span>
                            </div>
                          </div>
                          {!event.ok && event.message && (
                            <p className="text-xs text-red-700 mt-2 leading-relaxed">{event.message}</p>
                          )}
                          {event.ok && (
                            <p className="text-xs text-gray-500 mt-2 leading-relaxed">{eventOutputSummary(event)}</p>
                          )}
                        </div>
                      ))}
                    </div>
