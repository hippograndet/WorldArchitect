...
describe('core services on Postgres', () => {
  ...
  it('stores hosted provider settings per user and encrypts stored keys', async ({ skip }) => {
    ...
    const userA = await readEffectiveProviderSettings('user-a');
    const userB = await readEffectiveProviderSettings('user-b');
    ...
    const stored = await getDbClient().get<{ config: string }>(
...
async function clearData(): Promise<void> {
  ...
  await exec.run('DELETE FROM provider_settings');
